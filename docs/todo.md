...
- [ ] **知识点突破**
  ...
  - [ ] 了解 LangChain.js 核心抽象概览：Document Loader、Text Splitter、Embedding、Vector Store 各自的职责与边界
  - [x] 精读 LangChain.js 的 **Text Splitters** 模块（项目实际使用 `RecursiveCharacterTextSplitter`，理解 chunkSize / chunkOverlap 参数的影响）
  - [x] 阅读 LangChain.js 官方文档中的 "Retrieval" (检索) 章节，理解 RAG Pipeline 的标准组件与数据流
  - [x] 对比 LangChain Vector Store 抽象与自实现的差异（何时用 LangChain 封装、何时手写更灵活）
...
- [ ] **知识点突破**
  ...
  - [ ] 了解 LangChain.js 的 **Dynamic Tool / StructuredTool** 封装模式，对比手写 Tool Definition 与 LangChain 封装的优劣
  - [ ] 了解 Streaming + Tool Call 的前端 UX 处理方式（工具执行中 Loading 状态、中间步骤可视化）
  - [ ] 思考工具调用的错误处理与降级策略（调用失败、超时、模型拒绝执行时怎么办）
...
- [ ] **知识点突破**
  - [ ] 梳理两套框架的核心映射关系：`streamText` ↔ `.stream()`、`tool()` ↔ `DynamicTool`、`useChat` ↔ LangChain streaming response
  - [ ] 了解 LangChain 的模型抽象层（`ChatOpenAI`、`ChatGoogleGenerativeAI`），理解如何通过统一接口切换底层模型
  - [ ] 了解 LangChain 的 **Retrieval Chain** 封装（`createRetrievalChain` / `RunnablePassthrough`），对比手动拼接 System Prompt 的方式
  - [ ] 了解 LangChain **Agent Executor** 的工具调度机制，对比 Vercel AI SDK 的 `maxSteps` 自动循环
- [ ] **代码实战：对话层重构**
...
- [ ] **知识点突破**
  ...
  - [ ] 了解 **LCEL** (LangChain Expression Language) 的核心思想——Runnable 链式组合，理解 `pipe()` 与 `RunnableSequence` 如何将 LLM、Retriever、Parser 串联
  - [ ] 预研并阅读 **LangGraph.js** 官方教程的基础 State Graph 概念，理解图结构（State、Node、Edge）
  - [ ] 了解 LangChain **Callbacks / Tracing** 机制，理解如何在链式调用中插入日志、计时和自定义钩子
  - [ ] 了解 Agent 可观测性工具（如 LangSmith 或 Langfuse），学会追踪和调试多步工作流
  - [ ] 初步思考 Agent 输出质量评估方法（RAG 召回率、工具调用准确率、最终输出可用性）
- [ ] **代码实战：自主技术博文调研员**
...
- [ ] **知识点突破**
  ...
  - [ ] 了解主流 MCP Server 生态（文件系统、GitHub、数据库等现成 Server）
- [ ] **代码实战：接入 MCP Server**
